...
if __name__ == '__main__':
    img_dir = 'C:\\GraduationProject\\picture2\\'
    ...
    for i in range(len(img_name)):
        ...
        # 二值化
        result = cv2.adaptiveThreshold(grayImage, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 21, 1)  # 自适应阈值的选取
        # 去噪声
        ...
        cv2.imwrite('./progress-train2-picture/%s' % (img_name[i]), im_temp)
        print("%s %s" % (i, img_name[i]))
    print("图片预处理完成！")
